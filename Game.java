...
/**
 * A Game is a rectangular area containing a ball and two paddles (a Player
 * and a computer player). Only one instance of the Game class need be created in
 ...
 * attributes such as "bounds" which indicates the width and height of the
 * game area in pixels.
 * <p>
 ...
 * Game.State.WON, Game.State.LOST, Game.State.NEW_LEVEL. 
 * The current state of a game can be discovered by calling the "getState" method.
 * 
 * @author
 */
public class Game
{
    ...
    public Game()
    {
        try
        {
            myMusic = JApplet.newAudioClip(new URL("file:badgerbadger.wav"));
    ...
    /**
     * Starts the game. A separate thread will be started to repeatedly call
     * the tick() method in a loop.
     */
    ...
    /**
     * Returns true if the given area hits a paddle, or false
     * otherwise.
     *
     * @return true if the given area hits a paddle, or false
     * otherwise.
     */
    public boolean hitsPaddle(Rectangle area)
    {
        if( ((area.x + ball.SIZE == player.getArea().x) && (area.y >= player.getArea().y) && (area.y + ball.SIZE < player.getArea().y + player.getArea().height))
            || ((area.x == comp.getArea().x + comp.getArea().width) && (area.y >= comp.getArea().y) && (area.y + ball.SIZE < comp.getArea().y + comp.getArea().height)) )
        {
            myMusic.play();
            return true; 
        }
    ...
    /**
     * Puts the game into the Game.State.WON state. This method should be
    ...
    /**
     * Returns true if the given area is within the bounds of the game, or
     * false otherwise.
     *
     ...
     * monster should move one pixel. This method is called over and over
     * again by the game loop in the PlayThread.
     */
    public void tick()
    {
